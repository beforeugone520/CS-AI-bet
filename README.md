...
- Pick'em 策略更新：`3-0` 槽位现在对低样本/替补的 `player_availability_multiplier` 比普通 advance 更保守，因为 3-0 要求连续三场无失误，短期阵容波动的代价高于“最终晋级”。最终融合表的排序和 swing margin 已改用 `status_adjusted_score`，但这次状态调整没有改变最终选队，只把 MIBR/GamerLegion 这类低样本 `3-0` 的 raw score 下调到 adjusted score。这不会把低样本简单视为反向信号：当前 B8/BetBoom 这两个真实 3-0 同样存在低样本状态，因此状态项只降极端槽位的过度自信，不直接替代模型/市场/地图池判断。
