...
- player form 边界：原始 `forecast_report.json` 是 2026-06-01 赛前归档，不含 `player_form_summary`；重打标版本已从 player-form fixtures 补齐 8 场 form diff。由于当前真实 player form 样本置信度最高只有 0.2，`--player-form-counter-min-confidence 0.4` 没有触发反向自动规避，这比用低样本 form 直接改判更稳。
